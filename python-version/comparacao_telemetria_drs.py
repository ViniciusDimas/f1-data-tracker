...
def comparar_pilotos():
    print("\n" + "="*50)
    print("   COMPARADOR DE TELEMETRIA + ZONAS DE DRS   ")
    print("="*50)
    ...
    try:
        ...
        print(f"[2/4] Processando: {piloto1_code} (Volta {volta_p1_num}) vs {piloto2_code} (Volta {volta_p2_num})...")
        ...
        drs1 = np.interp(distancia_comum, tel1['Distance'], tel1['DRS'])
        drs_ativo = drs1 > 9 

        print("[3/4] Mapeando trechos de velocidade e zonas de DRS...")
        ...
        print("[4/4] Gerando gráfico final...")
        fig, ax = plt.subplots(figsize=(10, 8))
        fig.patch.set_facecolor('#1e1e1e')
        ax.set_facecolor('#1e1e1e')

        segments_drs = segments[drs_ativo[:-1]]
        if len(segments_drs) > 0:
            lc_drs = LineCollection(segments_drs, colors='#FFFF00', linewidth=11, alpha=0.8, zorder=1)
            ax.add_collection(lc_drs)

        cmap = ListedColormap([cor_p2, cor_p1])
        
        lc = LineCollection(segments, cmap=cmap, linewidth=4, zorder=2)
        ...
        ax.set_title(f"Domínio de Velocidade & DRS ({txt_volta}): {gp} {ano}\n{piloto1_code} vs {piloto2_code}", 
        ...
        legend_elements = [
            ...
            Line2D([0], [0], color='#FFFF00', lw=4, label='Zona de DRS Ativo')
        ]
        ...
        nome_arquivo = f"comparacao_drs_{piloto1_code}_vs_{piloto2_code}_{gp.lower()}_{ano}{sufixo_volta}.png".replace(" ", "_")
        plt.savefig(nome_arquivo, dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())

        print(f"\n🎉 Sucesso! Gráfico com zonas de DRS salvo como '{nome_arquivo}'.")
        print("Exibindo janela do gráfico...")
        plt.show()

    except Exception as e:
        print(f"\n[ERRO] Não foi possível gerar o gráfico de DRS. Detalhes: {e}")
